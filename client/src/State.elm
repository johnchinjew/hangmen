...
alphabetStateToString alphabet = 
    ...
    ( List.map 
        ( \pair -> 
            ...
            ++Tuple.first pair 
            ++ " : " 
            ++ if Tuple.second pair then "True" else "False"
        )
...
playerStateToString player =
    ...
    ++ player.word
    ++ " ready: " 
    ++ if player.ready then "True" else "False"
    ++ " alive: " 
    ++ if player.alive then "True }" else "False }"
...
sessionStateToString session =
    ...
    ++ " lobby: " 
    ++ if session.isLobby then "True" else "False"
    ++ " }"
